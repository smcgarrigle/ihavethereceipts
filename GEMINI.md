...
- **AI/OCR**:
    ...
    - **OCR Feedback Loop**: `app/services/correction_service.py` records human review corrections (`ocr_corrections` table) and builds the few-shot block injected into the receipt prompt by `process_receipt_task`, using only corrections from the same input type (photo, PDF or pasted text) and preferring the receipt's store. When changing the OCR prompt or models, measure the impact with `uv run python scripts/ocr_eval.py` (`--stored` = free baseline from past receipts, `--live` = re-run OCR with the corrections block production sends, minus the scored receipt's own corrections; `--corrections none` for the base prompt alone). Note: the OCR result cache is keyed on image + full prompt text, not the model, so move `data/ocr_cache` aside between models.
