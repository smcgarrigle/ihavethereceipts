---
trigger: always_on
---
# Agent Instructions & Blueprint

All AI agents working on this project must follow the specific coding styles, technology preferences, and workflow rules documented in [GEMINI.md](/grocery-tracker/GEMINI.md).

Core Rules:
- Create an implementation plan before writing code.
- Write reproducing tests for bugs before fixing.
- Prioritize dark mode and mobile responsiveness.
- Use HTMX for server-driven fragments and Alpine.js for client-side reactivity.
- Use `uv` for Python environment and package management.
